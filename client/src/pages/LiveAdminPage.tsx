...
export function LiveAdminPage() {
  ...
  return <AppShell title={title}><main className="live-admin-page" dir="rtl">
    ...
    {view === "overview" && <AdminOverview stats={access.data!} tickets={tickets.data} reports={reports.data} ticketsLoading={tickets.isLoading} reportsLoading={reports.isLoading}/>}
    {view === "users" && <UserTable users={users.data} isLoading={users.isLoading} onChange={(id, status) => userStatus.mutate({ id, status })} isSaving={userStatus.isPending}/>} 
    {view === "tickets" && <TicketTable tickets={tickets.data} isLoading={tickets.isLoading} onChange={(id, status) => ticketStatus.mutate({ id, status })} isSaving={ticketStatus.isPending}/>} 
    {view === "reports" && <ReportTable reports={reports.data} isLoading={reports.isLoading} onChange={(id, status) => reportStatus.mutate({ id, status })} isSaving={reportStatus.isPending}/>} 
  </main></AppShell>;
}

function AdminOverview({ stats, tickets, reports, ticketsLoading, reportsLoading }: { stats: { users: number; posts: number; communities: number; openTickets: number; openReports: number }; tickets?: ApiAdminTicket[]; reports?: ApiAdminReport[]; ticketsLoading: boolean; reportsLoading: boolean }) {
  ...
  return <><section className="live-admin-metrics">{metrics.map(([label, value, Icon]) => <Surface key={label}><Icon size={19}/><small>{label}</small><b>{value.toLocaleString("ar-YE")}</b><span>بيانات حية</span></Surface>)}</section><section className="live-admin-split"><AdminPreview icon={TicketCheck} title="أحدث طلبات الدعم" href="/admin/tickets" loading={ticketsLoading}>{tickets?.slice(0, 4).map((ticket) => <div key={ticket.id}><PersonLabel user={ticket.user}/><span>{labelFor(ticket.status, ticketStatuses)}</span></div>)}</AdminPreview><AdminPreview icon={FileWarning} title="أحدث البلاغات" href="/admin/reports" loading={reportsLoading}>{reports?.slice(0, 4).map((report) => <div key={report.id}><b>{report.reason}</b><span>{labelFor(report.status, reportStatuses)}</span></div>)}</AdminPreview></section></>;
}

function AdminPreview({ icon: Icon, title, href, loading, children }: { icon: typeof TicketCheck; title: string; href: string; loading: boolean; children?: React.ReactNode }) {
  return <Surface className="live-admin-preview"><header><span><Icon size={18}/></span><b>{title}</b><Link href={href}>عرض الكل</Link></header>{loading ? <p className="live-admin-muted">جارٍ تحميل البيانات…</p> : children ? <div className="live-admin-preview-list">{children}</div> : <p className="live-admin-muted">لا توجد سجلات حية لعرضها الآن.</p>}</Surface>;
}

function UserTable({ users, isLoading, onChange, isSaving }: { users?: ApiAdminUser[]; isLoading: boolean; onChange: (id: string, status: UserStatus) => void; isSaving: boolean }) {
  return <DataSection icon={UsersRound} title="المستخدمون" loading={isLoading} empty="لا توجد حسابات متاحة في الاستجابة الحالية.">{users?.map((user) => <div className="live-admin-row" key={user.id}><PersonLabel user={user}/><small>{user.email ?? "لا يوجد بريد معروض"}</small><small>{user.lastLoginAt ? `آخر دخول ${asRelativeTime(user.lastLoginAt)}` : "لا توجد بيانات دخول"}</small><select aria-label={`حالة ${user.displayName || user.username || user.id}`} value={user.status || "ACTIVE"} disabled={isSaving} onChange={(event) => onChange(user.id, event.target.value as UserStatus)}>{userStatuses.map((status) => <option value={status.value} key={status.value}>{status.label}</option>)}</select></div>)}</DataSection>;
}

function TicketTable({ tickets, isLoading, onChange, isSaving }: { tickets?: ApiAdminTicket[]; isLoading: boolean; onChange: (id: string, status: TicketStatus) => void; isSaving: boolean }) {
  return <DataSection icon={TicketCheck} title="طلبات الدعم" loading={isLoading} empty="لا توجد طلبات دعم في الاستجابة الحالية.">{tickets?.map((ticket) => <div className="live-admin-row" key={ticket.id}><div><b>{ticket.subject}</b><small>{ticket.category}</small></div><PersonLabel user={ticket.user}/><small>{asRelativeTime(ticket.createdAt)}</small><select aria-label={`حالة طلب ${ticket.subject}`} value={ticket.status} disabled={isSaving} onChange={(event) => onChange(ticket.id, event.target.value as TicketStatus)}>{ticketStatuses.map((status) => <option value={status.value} key={status.value}>{status.label}</option>)}</select></div>)}</DataSection>;
}

function ReportTable({ reports, isLoading, onChange, isSaving }: { reports?: ApiAdminReport[]; isLoading: boolean; onChange: (id: string, status: ApiAdminReport["status"]) => void; isSaving: boolean }) {
  return <DataSection icon={FileWarning} title="بلاغات المحتوى" loading={isLoading} empty="لا توجد بلاغات في الاستجابة الحالية.">{reports?.map((report) => <div className="live-admin-row" key={report.id}><div><b>{report.reason}</b>{report.details && <small>{report.details}</small>}</div><PersonLabel user={report.reporter}/><small>{asRelativeTime(report.createdAt)}</small><select aria-label={`حالة البلاغ ${report.reason}`} value={report.status} disabled={isSaving} onChange={(event) => onChange(report.id, event.target.value as ApiAdminReport["status"])}>{reportStatuses.map((status) => <option value={status.value} key={status.value}>{status.label}</option>)}</select></div>)}</DataSection>;
}

function DataSection({ icon: Icon, title, loading, empty, children }: { icon: typeof UsersRound; title: string; loading: boolean; empty: string; children?: React.ReactNode }) {
  return <Surface className="live-admin-table"><header><span><Icon size={19}/></span><div><h2>{title}</h2><p>بيانات مباشرة مقيدة بصلاحية المدير.</p></div></header>{loading ? <p className="live-admin-muted">جارٍ تحميل البيانات…</p> : children ? <div className="live-admin-list">{children}</div> : <p className="live-admin-muted">{empty}</p>}</Surface>;
}
