...
import { Activity, ArrowLeft, BarChart3, Bell, Bot, BrainCircuit, Check, ChevronLeft, CircleAlert, CircleHelp, Clock3, FileQuestion, Headphones, LifeBuoy, LockKeyhole, Mail, MessageCircle, Phone, Search, Send, ShieldAlert, ShieldCheck, Sparkles, Trash2, UserRound, UserRoundCheck, UserRoundCog, UserRoundX, UsersRound, Zap } from "lucide-react";
...
export function AccountSuitePage() {
  ...
  const submitProfile = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    updateProfile.mutate({ displayName: String(values.get("displayName") || ""), username: String(values.get("username") || ""), bio: String(values.get("bio") || "") });
  };
  ...
  return <AppShell title={title}><main className="completion-page account-suite">
    {mode === "overview" && <><SuiteHead eyebrow="حسابك" title="إدارة معلومات حسابك" text="تحكم في بيانات التعريف، طرق الاسترداد، وخيارات حالة الحساب."/><section className="account-nav">{accountLinks.map(([label,text,href,Icon])=>{const I=Icon as typeof UserRound; return <Link href={href as string} key={label as string}><i><I/></i><div><strong>{label as string}</strong><p>{text as string}</p></div><ChevronLeft/></Link>})}</section><section className="account-danger"><div><UserRoundX/><div><strong>إيقاف أو حذف الحساب</strong><p>اختر التعطيل المؤقت أو الحذف النهائي وفق احتياجك.</p></div></div><div><Link href="/account/disable">تعطيل</Link><Link href="/account/delete">حذف الحساب</Link></div></section></>}
    {mode === "info" && <>{!signedIn ? <div className="empty-completion"><LockKeyhole/><h2>سجّل الدخول لعرض معلومات حسابك</h2><Link href="/login" className="crimson-button">تسجيل الدخول</Link></div> : profile.isLoading ? <div className="empty-completion"><Clock3 className="animate-spin"/><p>يجري تحميل بيانات الحساب…</p></div> : profile.isError ? <div className="empty-completion"><CircleAlert/><p>تعذر تحميل معلومات الحساب.</p><button className="quiet-button" onClick={()=>profile.refetch()}>إعادة المحاولة</button></div> : <><SuiteHead eyebrow="حسابك" title="معلومات الحساب" text="بيانات التعريف الأساسية المرتبطة بحساب يمنا."/><section className="account-profile-card"><Avatar person={people[0]} size="xl"/><div><h2>{account?.displayName || account?.fullName || "حساب يمنا"}</h2><p>@{account?.username || "—"}{account?.createdAt ? ` · عضو منذ ${new Date(account.createdAt).toLocaleDateString("ar-YE", { month: "long", year: "numeric" })}` : ""}</p><span><ShieldCheck/> {account?.status === "ACTIVE" ? "الحساب نشط" : "البيانات محمية داخل يمنا"}</span></div></section><dl className="info-list"><div><dt>البريد الإلكتروني</dt><dd>{account?.email || "غير مضاف"}</dd></div><div><dt>رقم الهاتف</dt><dd>{account?.phone || "غير مضاف"}</dd></div><div><dt>اللغة</dt><dd>العربية</dd></div><div><dt>المنطقة</dt><dd>{[account?.city, account?.governorate].filter(Boolean).join("، ") || "غير محددة"}</dd></div></dl></>}</>}
    {mode === "edit" && <form className="completion-form" onSubmit={submitProfile}><SuiteHead eyebrow="ملفك" title="تعديل البيانات" text="عدّل المعلومات التي تظهر للمجتمع."/>{!signedIn ? <div className="empty-completion"><LockKeyhole/><h2>سجّل الدخول لتعديل ملفك</h2><Link href="/login" className="crimson-button">تسجيل الدخول</Link></div> : profile.isLoading ? <div className="empty-completion"><Clock3 className="animate-spin"/><p>يجري تحميل بيانات الملف…</p></div> : <><label>الاسم<input name="displayName" required minLength={2} defaultValue={account?.displayName || account?.fullName || ""}/></label><label>اسم المستخدم<input name="username" required minLength={3} defaultValue={account?.username || ""}/></label><label>نبذة<textarea name="bio" defaultValue={account?.bio || ""}/></label><button disabled={updateProfile.isPending} className="crimson-button" type="submit">{updateProfile.isPending ? "جارٍ الحفظ…" : saved ? <><Check/> تم الحفظ</> : "حفظ التغييرات"}</button></>}</form>}
...
export function RelationsCompletionPage() {
  ...
  return <AppShell title={mode === "mutual" ? "الأصدقاء المشتركون" : "إدارة العلاقة"}><main className="completion-page relations-completion">
    {mode === "mutual" && <><SuiteHead eyebrow="علاقاتك" title="أصدقاء مشتركون" text="أشخاص تعرفهم أنت ومحمد الحاج، وقد يساعدونكما على بدء تواصل جديد."/><section className="mutual-hero"><Avatar person={people[0]} size="lg"/><div className="mutual-people">{mutualPeople.map(person=><Avatar key={person.id} person={person} size="md"/>)}</div><strong>3 أصدقاء مشتركون</strong></section><section className="people-list">{mutualPeople.map((person, i)=><article key={person.id}><Avatar person={person} size="lg"/><div><strong>{person.name}</strong><p>{i + 2} أصدقاء مشتركون · {i === 0 ? "يعمل في التعليم" : "يهتم بالمجتمعات"}</p></div><Link href="/profile" className="quiet-button">عرض الملف</Link></article>)}</section></>}
