...
const mutualPeople: typeof people = [];
export function RelationsCompletionPage() {
  ...
  return <AppShell title={mode === "mutual" ? "الأصدقاء المشتركون" : "إدارة العلاقة"}><main className="completion-page relations-completion">
    {mode === "mutual" && (!signedIn ? <div className="empty-completion"><LockKeyhole/><h2>سجّل الدخول لعرض الأصدقاء المشتركين</h2><Link href="/login" className="crimson-button">تسجيل الدخول</Link></div> : isCurrentUserLoading ? <div className="empty-completion"><Clock3 className="animate-spin"/><p>يجري تحميل حسابك…</p></div> : !currentPerson ? <div className="empty-completion"><CircleAlert/><p>تعذر تحميل حسابك لعرض العلاقات المشتركة.</p></div> : <><SuiteHead eyebrow="علاقاتك" title="أصدقاء مشتركون" text="تظهر هنا الحسابات المشتركة بين الحسابين عند توفر بيانات علاقات حقيقية."/><section className="mutual-hero"><Avatar person={currentPerson} size="lg"/><div className="mutual-people">{mutualPeople.map(person=><Avatar key={person.id} person={person} size="md"/>)}</div><strong>{mutualPeople.length} أصدقاء مشتركون</strong></section>{mutualPeople.length ? <section className="people-list">{mutualPeople.map((person, i)=><article key={person.id}><Avatar person={person} size="lg"/><div><strong>{person.name}</strong><p>{i + 2} أصدقاء مشتركون</p></div><Link href={`/profile/${encodeURIComponent(person.username || String(person.id))}`} className="quiet-button">عرض الملف</Link></article>)}</section> : <div className="empty-completion"><UsersRound/><h2>لا توجد بيانات أصدقاء مشتركة بعد</h2><p>ستظهر هنا الحسابات المشتركة عند توفر علاقة حقيقية بين الحسابين.</p></div>}</>)}
