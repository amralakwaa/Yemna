...
| العلاقات | `/friends`، `/friends/mutual`، `/friend-requests`، `/followers`، `/following`، `/blocked`، `/friendship/manage` | موجودة | صفحة الأصدقاء الأساسية موصولة بـ REST؛ تبقى الصفحات التفصيلية الأخرى للربط التدريجي |
| المحادثات والإشعارات | `/messages`، `/messages/new`، `/messages/chat`، `/messages/group`، `/notifications`، `/notifications/settings` | موجودة | صفحتا الرسائل والإشعارات الأساسيتان موصولتان بـ REST والاتصال اللحظي؛ تبقى الصفحات التفصيلية للربط التدريجي |
| المجتمعات المحلية | `/communities`، `/communities/local`، `/communities/governorates`، `/communities/universities`، `/communities/interests`، `/communities/nearby`، `/community/*` | موجودة | صفحة قائمة المجتمعات الأساسية موصولة بـ REST؛ تبقى صفحات الاستكشاف التفصيلية للربط التدريجي |
...
| البحث والاكتشاف | `/search`، `/search/advanced`، `/search/voice`، `/discover`، `/discover/map`، `/discover/interests` | موجودة | صفحة البحث الأساسية موصولة بـ REST؛ تبقى أدوات البحث والاكتشاف المتخصصة للربط التدريجي |
| الإدارة والإشراف | `/admin`، `/admin/users`، `/admin/content`، `/admin/reports`، `/admin/analytics`، `/admin/logs`، `/admin/ai-analytics` | موجودة | API الإدارة محمي بدور `ADMIN`، وجميع مسارات `/admin` محروسة في الواجهة بفحص صلاحية قبل عرض أي بيانات أو عناصر توضيحية |
...
| الإدارة | `/admin` | مقيّد | لا تُعرض لوحة أو مؤشرات الإدارة قبل نجاح فحص صلاحية المدير. |
| الإدارة | `/admin/users` | مقيّد | لا تُعرض قوائم المستخدمين أو عناصر توضيحية قبل نجاح فحص صلاحية المدير. |
| الإدارة | `/admin/content` | مقيّد | مسار الإدارة محروس قبل عرض واجهة مراقبة المحتوى. |
| الإدارة | `/admin/reports` | مقيّد | مسار الإدارة محروس قبل عرض واجهة البلاغات أو التذاكر. |
| الإدارة | `/admin/logs` | مقيّد | مسار الإدارة محروس قبل عرض سجل العمليات. |
| الإدارة | `/admin/ai-analytics` | مقيّد | مسار تحليلات الذكاء الاصطناعي محروس بالحارس نفسه. |
| الإدارة — دون جلسة | `/admin` | مرّ | عند عرض الهاتف `390×844` تظهر رسالة «تسجيل الدخول مطلوب» كاملة داخل بطاقة متمركزة، دون مؤشرات أو بيانات توضيحية. |
| الإدارة — دون جلسة | `/admin/users` | مرّ | عند عرض الهاتف `390×844` تظهر رسالة تسجيل الدخول نفسها كاملة داخل الإطار، دون عرض سجلات المستخدمين أو تجاوز أفقي. |
| الإدارة — مستخدم عادي (`403`) | `/admin` | مرّ | عند عرض الهاتف `390×844` بحساب اختبار عادي تظهر رسالة «لا يمكن فتح لوحة الإدارة» كاملة، مع تأكيد عدم عرض المؤشرات أو البيانات الإدارية. |
| الإدارة — مستخدم عادي (`403`) | `/admin/users` | مرّ | عند عرض الهاتف `390×844` بحساب اختبار عادي تظهر رسالة المنع نفسها كاملة، دون عرض قائمة المستخدمين أو تجاوز أفقي. |
...
| 18: الإدارة | لوحة الإحصاءات | `/admin` | مرّ — البيانات الحية تتطلب دور المدير |
| 18: الإدارة | تحليل البلاغات | `/admin/reports` | مرّ — حالة عدم الجلسة موثقة؛ البيانات الحية تتطلب دور المدير |
| 18: الإدارة | مراقبة المستخدمين | `/admin/users` | مرّ — حالة عدم الجلسة موثقة؛ البيانات الحية تتطلب دور المدير |
...
| `/search` | أعاد البحث خطأ 500 لأن `SearchService` لم يكن محقوناً في متحكم البحث، كما اختلفت مفاتيح نتائج الفلاتر عن عقد الواجهة. | ثُبّت حقن `SearchService` وطُبّع عقد النتائج لتضم الاستجابة القوائم الثلاث دائماً. | اجتازت اختبارات الخدمة والمتحكم واستجابة REST للفلاتر الثلاث، ثم اختبار تدفق البحث التفاعلي. |
...
## تحقق تفاعلي من تدفقات المستخدم

إضافة إلى فحص الهاتف، يغطي الاختبار القابل لإعادة التشغيل `client/src/components/yemna/UserFlows.test.tsx` النقرات الفعلية التالية في متصفح وهمي: الانتقال من العلاقات إلى الإشعارات، فتح قائمة الهاتف ثم الانتقال إلى البحث، تسجيل الدخول الناجح والعودة إلى التغذية مع حفظ رمز الجلسة، ثم البحث الحي والانتقال من النتيجة إلى الملف العام أو استكشاف مجتمع مطابق. يعتمد الاختبار استجابات REST محاكية محصورة ضمن بيئة الاختبار، ولا يضيف بيانات تجريبية إلى قاعدة الإنتاج.

| الأمر | النتيجة المسجلة |
|---|---|
| `pnpm vitest run client/src/components/yemna/UserFlows.test.tsx` | مرّ: 3 اختبارات تدفق تفاعلي |

## تدقيق توحيد الواجهة والتدفقات بعد الربط

راجعنا على هاتف بعرض `390×844` صفحات الدعم والحساب والوسائط والإدارة بعد أحدث عمليات الربط والحراسة. تستخدم صفحات الدعم والحساب والوسائط الغلاف العربي المشترك، والطباعة نفسها، والسطح الأبيض فوق الخلفية الرمادية الهادئة، والعنابي كإشارة للإجراء أو الحالة النشطة. بقيت بطاقات الدعم والحساب وظيفية ومقروءة، كما ظلت صفحة الوسائط في سياق المحتوى الاجتماعي نفسه بدلاً من أن تصبح شاشة إعدادات منفصلة.

| النطاق | نتيجة التدقيق | الإجراء أو الدليل |
|---|---|---|
| `/help` و`/account` | مرّ بعد تحسين | أضيفت كتلة رأسية هادئة ذات هرمية أوضح، وفُصلت ألوان الأيقونات بحسب معنى الإجراء بدلاً من تكرار العنابي كزخرفة؛ بقيت الإجراءات الحساسة هي التي تستخدم العنابي بوضوح. |
| `/media` | مرّ | التبويبات والمحتوى وحالات الحساب تستخدم عناصر يمنا المشتركة وتبقى قابلة للقراءة على الهاتف. |
| `/admin` و`/admin/users` بلا جلسة | مرّ بعد إصلاح وتحسين | توحّدت بطاقة المنع مع لغة يمنا عبر شارة «مساحة موثوقة» وحد علوي عنابي، وتظهر بطاقة «تسجيل الدخول مطلوب» متمركزة وكاملة على عرض `390×844` دون عرض أي بيانات إدارية. |
| تدفقات التنقل والحراسة | مرّ | يثبت `UserFlows.test.tsx` انتقال العلاقات والدرج المحمول والبحث والروابط التالية، ويثبت `AdminAISuite.test.tsx` عدم عرض بيانات الإدارة عند غياب الجلسة أو عند رفض الصلاحية. |
...
## تدفقات المستخدم المرجعية

تستخدم الشاشات العامة قشرة التطبيق الموحدة ذات الاتجاه RTL والتنقل السفلي على الهاتف والتنقل الجانبي على الشاشات الأوسع. يظل الوصول إلى المسارات التخصصية قابلاً للرجوع عبر عنوان الصفحة والتنقل الأساسي، فيما توجه التدفقات المحمية المستخدم غير المصادق إلى تسجيل الدخول بدلاً من عرض بيانات محلية توحي بنجاح عملية غير منفذة.

| التدفق | نقطة البداية | المسار الوظيفي | النتيجة المتحققة |
|---|---|---|---|
| الانضمام والوصول | `/register` أو `/login` | إنشاء حساب أو تسجيل الدخول ثم العودة إلى `/` | تخزين رمز الجلسة في العميل وعرض التغذية الحية أو حالات الخطأ الصريحة |
| إنشاء المحتوى | `/` | مؤلف المنشور ثم `POST /posts` | تحديث تغذية REST بعد النشر، أو تنبيه يطلب الدخول عند غياب الجلسة |
| اكتشاف المستخدمين والمجتمعات | `/search` أو `/communities` | بحث مباشر أو استعراض مجتمع ثم طلب الانضمام | عقد بحث ثابت لكل مرشح، وحالات تحميل وفراغ وخطأ، مع منع الانضمام دون جلسة |
| العلاقات والتواصل | `/friends` ثم `/messages` و`/notifications` | استعراض العلاقات أو معالجة الطلبات أو قراءة المحادثات والإشعارات | حماية الجلسة وإتاحة حالات واضحة للمستخدم غير المصادق، مع أحداث اتصال لحظية للمستخدم المصادق |
| الحساب والدعم | `/account` أو `/help` | الانتقال إلى المعلومات أو الخصوصية أو البلاغ أو التواصل | صفحات فرعية مستقلة ضمن نفس الهوية المرئية ومسار رجوع واضح |
| الإدارة | `/admin` | نظرة عامة ثم المستخدمون أو المحتوى أو البلاغات أو السجلات | واجهات إدارية متسقة بصرياً؛ تظل البيانات الإدارية الحية محكومة بدور الإدارة في الـ API |

> لا تعني المراجعة المرئية أن كل إجراء إداري أو خاص بالحساب نفذ ببيانات إنتاجية؛ فهي تثبت وجود المسارات واتساق انتقالها، بينما يغطي الاختبار الخلفي عمليات REST والوصول المحمي المتاحة حالياً.

### تحقق الوصول الإداري على الهاتف

بعد إصلاح احتواء قشرة الإدارة والجداول عند عرض الهاتف، التقط الفحص الهاتفي لمساري `/admin/users` و`/admin/reports` بلا جلسة، وتظهر فيهما دعوة تسجيل دخول المدير كاملة من دون سجلات خاصة. كما يغطي الاختبار التفاعلي `client/src/pages/AdminAISuite.test.tsx` حالتي غياب الجلسة و`403` بطلبات مطابقة لعقود الإدارة.

ثبت التحقق التشغيلي بحساب اختبار عادي جديد: قبل الخادم الطلب إلى `/admin/users` ثم أعاد منع الصلاحية، وعرضت الواجهة نصي **«تعذر عرض البيانات الإدارية»** و**«حسابك لا يحمل صلاحية المدير»** من دون إظهار سجلات محمية. لا يتضمن هذا السجل أي بيانات اعتماد أو رموز جلسة. تفصل بيئة الفحص الهاتفي جلسة المعاينة عن جلسة المتصفح المستخدمة للتحقق الحي؛ لذلك يسجل الدليل بوضوح لقطة هاتفية لغياب الجلسة مع تحقق `403` حي مستقل، ولا يدّعي لقطة موحدة غير ملتقطة.

على الهاتف، تبقى **هوية السجل وحالته وإجراءه** ظاهرة داخل حاوية الجدول، بينما تُخفى الأعمدة الوصفية الثانوية لتفادي تجاوز العرض. لا يغير ذلك حمولات REST أو صلاحيات الإجراءات؛ إذ يستمر زر التحديث في استخدام العقد المحمي نفسه، ولا يظهر إلا عندما تتاح بيانات مدير فعلية.
